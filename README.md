...
- 进入 READY、RUNNING时 reset 全部控制 PID；STABILIZE/ALT_HOLD切换只重置高度环。
...
- BMP388与BMI088已接入可切换的固定互补、三状态卡尔曼和动态权重互补高度估计；当前默认三状态卡尔曼，ALT_HOLD仍需按检查表完成实机调参。
- ALT_HOLD支持1490us悬停前馈、油门捕获区以及进入/退出的无扰混合；不包含独立的一键自动起飞/降落轨迹。
- FAST_TELEMETRY v1.2负载为46字节：在原42字节后追加目标高度int32，单位0.01m；有效性位图bit 8表示目标高度当前有效。
